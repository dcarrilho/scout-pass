          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {moto.owned_until === null && moto.owned_from !== null && (
                <span className="text-xs rounded-full px-2 py-0.5 font-medium" style={{ background: "rgba(249,115,22,0.15)", color: "#f97316" }}>
                  Atual
                </span>
              )}
            </div>
...
export default function MotorcycleForm({ motorcycles }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Adicionar nova moto */}
      <div className="rounded-xl p-4 space-y-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px dashed rgba(255,255,255,0.12)" }}>
        <p className="text-sm font-semibold text-white/60 uppercase tracking-wider">Adicionar moto</p>
        <form action={action} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <DarkField label="Marca" error={state?.errors?.brand?.[0]}>
